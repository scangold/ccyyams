...
<body class="no-skin">
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="archivestorage/realstoragelist.do" method="post" name="SForm" id="SForm">
								...
								<table id="simple-table"
									...
									<thead>
										<tr>
											...
											<th class="center" style="width:50px;">存储ID</th>
											<th class="center">存储类型名称</th>
											<th class="center">存储名称</th>
											<th class="center">存储类型</th>
											<th class="center">容量</th>
											<th class="center">可用容量</th>
											<th class="center">单位</th>
											<th class="center">位置</th>
											<!-- <th class="center">保管时间</th>
											<th class="center">删除时间</th>
											<th class="center">删除原因</th>
											<th class="center">序列号</th>
											<th class="center">案卷年限</th>
											<th class="center">档案室编</th>
											<th class="center">档案馆编</th>
											<th class="center">档案页数</th> -->
												...
												<c:forEach items="${varList}" var="var" varStatus="vs">
													<tr>
														<td class='center'><label class="pos-rel"><input
																type='checkbox' name='ids' value="${var.REALSTORAGE_ID}"
																class="ace" /><span class="lbl"></span></label></td>
														<td class='center' style="width: 30px;">${vs.index+1}</td>
														<td class='center' style="width: 30px;">${var.REALSTORAGE_ID}</td>
														<td class='center'>${var.STORAGE_TYPENAME}</td>
														<td class='center'>${var.STORAGE_NAME}</td>
														<td class='center'>${var.STORAGE_TYPE}</td>
														<td class='center'>${var.VOLUME}</td>
														<td class='center'>${var.ACTVOLUME}</td>
														<td class='center'>${var.UNIT}</td>
														<td class='center'>${var.LOCATION}</td>
														<%-- <td class='center'>${var.LIMIT}</td>
														<td class='center'>${var.DELETETIME}</td>
														<td class='center'>${var.DELETEWHY}</td>
														<td class='center'>${var.SERANUM}</td>
														<td class='center'>${var.FD_YEAR}</td>
														<td class='center'>${var.FD_S}</td>
														<td class='center'>${var.FD_G}</td>
														<td class='center'>${var.FD_PAGE}</td> --%>
														<td class='center'>${var.OTHER1}</td>
														<td class='center'>${var.STORAGE_REMARK}</td>
														...
														<td class="center"><c:if
															...
															<div class="hidden-sm hidden-xs btn-group">
																<%-- <c:if test="${QX.edit == 1 }">
																<a class="btn btn-xs btn-success" title="编辑" onclick="editS('${user.USER_ID}');">
																	<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
																</a>
																</c:if> --%>
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="editstorage('${var.REALSTORAGE_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
																<c:if test="${QX.del == 1 }">
																	<a class="btn btn-xs btn-danger"
																		onclick="deleteS('${var.REALSTORAGE_ID }','${var.STORAGE_NAME }');">
														    ...
														    <c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="editstorage('${var.REALSTORAGE_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
																		<i class="ace-icon fa fa-pencil-square-o bigger-120"></i>
																	</span>
																</a>
																		...
																		<c:if test="${QX.del == 1 }">
																			<li><a style="cursor:pointer;"
																				onclick="deleteS('${var.REALSTORAGE_ID }','${var.STORAGE_ENAME }');"
									...
									<table style="width:100%;">
										<tr>
											<td style="vertical-align:top;"><c:if
													test="${QX.add == 1 }">
													<a class="btn btn-sm btn-success" onclick="addS();">新增</a>
												</c:if>
												<!--<c:if test="${QX.del == 1 }">
	...
	<script type="text/javascript">
		...
		function addS(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增存储介质";
			 diag.URL = '<%=basePath%>archivestorage/goAdd.do';
			 diag.Width = 600;
			 diag.Height = 400;
		...
		//删除
		function deleteS(realstorageId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>archivestorage/deleteS.do?REALSTORAGE_ID="+realstorageId+"&tm="+new Date().getTime();
		...
		//修改
function editstorage(realstorage_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>archivestorage/goEditstorage.do?REALSTORAGE_ID='+realstorage_id;
	 diag.Width = 469;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
